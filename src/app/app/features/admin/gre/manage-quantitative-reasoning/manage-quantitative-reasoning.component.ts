...
@Component({
  selector: 'app-manage-quantitative-reasoning',
  standalone: true,
  imports: [CommonModule, RouterModule, ReactiveFormsModule],
  templateUrl: './manage-quantitative-reasoning.component.html',
  styleUrls: ['./manage-quantitative-reasoning.component.scss']
})
export class ManageQuantitativeReasoningComponent implements OnInit {
  quantitativeQuestions: QuestionDto[] = [];
  ...
  readonly QUANTITATIVE_EXAM_ID = 18;
  ...
  ngOnInit(): void {
    this.loadQuantitativeQuestions();
    this.initForm();
    this.addOption();
  }
  ...
  initForm() {
    this.questionForm = this.fb.group({
      examId: [this.QUANTITATIVE_EXAM_ID, Validators.required],
  ...
  // ✅ load only quantitative questions
  loadQuantitativeQuestions() {
    this.questionService.getAll().subscribe(qs => {
      this.quantitativeQuestions = qs.filter(q => q.examId === this.QUANTITATIVE_EXAM_ID);
  ...
  onSubmit() {
    ...
    if (this.isEditing && this.editingQuestionId) {
      const updateDto: UpdateQuestionDto = {
        ...
        examId: this.QUANTITATIVE_EXAM_ID,
  ...
  // ✅ edit QUANTITATIVE question (added explanation)
  ...
  deleteQuestion(id: number) {
    if (confirm('Are you sure you want to delete this Gre Quantitative Reasoning question?')) {
