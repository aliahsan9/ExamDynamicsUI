...
@Component({
  selector: 'app-manage-verbal-reasoning',
  standalone: true,
  imports: [CommonModule, RouterModule, ReactiveFormsModule],
  templateUrl: './manage-verbal-reasoning.component.html',
  styleUrls: ['./manage-verbal-reasoning.component.scss']
})
export class ManageVerbalReasoningComponent implements OnInit {
  verbalQuestions: QuestionDto[] = [];
  ...
  initForm() {
    this.questionForm = this.fb.group({
      examId: [this.VERBAL_EXAM_ID, Validators.required],
  ...
  // ✅ load only verbal questions
  loadVerbalQuestions() {
    this.questionService.getAll().subscribe(qs => {
      this.verbalQuestions = qs.filter(q => q.examId === this.VERBAL_EXAM_ID);

      this.verbalQuestions.forEach(q => {
  ...
  // ✅ edit Verbal question (added explanation)
  ...
  deleteQuestion(id: number) {
    if (confirm('Are you sure you want to delete this Gre Verbal Reasoning question?')) {
